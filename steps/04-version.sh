...
# Replaces the first 'version = "..." line in a Cargo.toml with the given version.
patch_cargo_version() {
    local file="$1"
    local version="$2"
...
# Patches multiple Cargo.toml files and Tauri config with persistent versions.
patch_cargo_versions_on_exit() {
    local version="${!#}"
    local files=("Cargo.toml" "base-src/Cargo.toml" "bridge-src-tauri/Cargo.toml")

    echo "Set Cargo.toml version -> $version."
    for f in "${files[@]}"; do
        patch_cargo_version "$f" "$version"
    done
